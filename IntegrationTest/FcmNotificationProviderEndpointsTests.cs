...
public sealed class FcmNotificationProviderEndpointsTests
{
    ...
    [Fact]
    public async Task OutboxRunWithEmailEnabledMarksAttemptEmailSent()
    {
        var stores = new Stores();
        stores.Attempts.Items.Add(new NotificationDeliveryAttempt { Id = "email-attempt", UserId = "rider-owner", AlertDispatchId = "alert", IncidentId = "incident", EmergencyContactId = "contact", ContactEmail = "monitor@example.com", Channel = NotificationChannel.Email, Status = NotificationDeliveryStatus.Prepared, PreparedAtUtc = Now, CreatedAtUtc = Now });
        await using WebApplicationFactory<Program> factory = CreateFactory(stores, enabled: false, emailEnabled: true);
        HttpClient admin = factory.CreateClient(); await AuthenticateAsync(admin, "email-admin@example.com", UserRole.Admin, stores);

        RunEnvelope run = (await (await admin.PostAsJsonAsync("/api/v1/admin/notifications/outbox/run", new RunNotificationOutboxRequest(20, false))).Content.ReadFromJsonAsync<RunEnvelope>())!;

        run.Data.SimulatedSent.Should().Be(1);
        stores.Attempts.Items[0].Provider.Should().Be(NotificationProvider.Email);
        stores.Attempts.Items[0].Status.Should().Be(NotificationDeliveryStatus.SimulatedSent);
        stores.EmailSender.Calls.Should().Be(1);
        stores.EmailSender.LastMessage!.Body.Should().Contain("incidentId: incident").And.NotContain("token");
    }

    [Fact]
    public async Task OutboxRunWithEmailFailureMarksAttemptEmailFailed()
    {
        var stores = new Stores(); stores.EmailSender.Fail = true;
        stores.Attempts.Items.Add(new NotificationDeliveryAttempt { Id = "email-attempt", UserId = "rider-owner", AlertDispatchId = "alert", IncidentId = "incident", EmergencyContactId = "contact", ContactEmail = "monitor@example.com", Channel = NotificationChannel.Email, Status = NotificationDeliveryStatus.Prepared, PreparedAtUtc = Now, CreatedAtUtc = Now });
        await using WebApplicationFactory<Program> factory = CreateFactory(stores, enabled: false, emailEnabled: true);
        HttpClient admin = factory.CreateClient(); await AuthenticateAsync(admin, "email-admin2@example.com", UserRole.Admin, stores);
        ...
        stores.Attempts.Items[0].FailureReason.Should().Be("email_provider_failed");
    }
    ...
    private static WebApplicationFactory<Program> CreateFactory(Stores stores, bool enabled, string? serviceAccountJson = "{}", string? serviceAccountJsonBase64 = null, bool emailEnabled = false) => new WebApplicationFactory<Program>().WithWebHostBuilder(builder => { builder.UseEnvironment("Testing"); builder.ConfigureAppConfiguration((_, c) => c.AddInMemoryCollection(new Dictionary<string, string?> { ["Jwt:Issuer"] = "MotoSOS", ["Jwt:Audience"] = "MotoSOS.Clients", ["Jwt:Key"] = new string('F', 48), ["Jwt:AccessTokenMinutes"] = "15", ["Jwt:RefreshTokenDays"] = "7", ["Jwt:RefreshTokenRememberMeDays"] = "30", ["MongoDb:" + "Connection" + "String"] = string.Empty, ["MongoDb:DatabaseName"] = "MotoSOS_Test", ["Notifications:Providers:Fcm:Enabled"] = enabled.ToString(), ["Notifications:Providers:Fcm:ProjectId"] = enabled ? "test-project" : null, ["Notifications:Providers:Fcm:ServiceAccountJson"] = enabled ? serviceAccountJson : null, ["Notifications:Providers:Fcm:ServiceAccountJsonBase64"] = enabled ? serviceAccountJsonBase64 : null, ["Notifications:Providers:Email:Enabled"] = emailEnabled.ToString(), ["Notifications:Providers:Email:FromEmail"] = emailEnabled ? "alerts@example.com" : null, ["Notifications:Providers:Email:FromName"] = emailEnabled ? "MotoSOS" : null, ["Notifications:Providers:Email:SmtpHost"] = emailEnabled ? "smtp.example.test" : null, ["Notifications:Providers:Email:SmtpPort"] = emailEnabled ? "587" : null, ["Notifications:Providers:Email:SmtpUsername"] = emailEnabled ? "smtp-user" : null, ["Notifications:Providers:Email:SmtpPassword"] = emailEnabled ? "smtp-secret" : null, ["Notifications:Providers:Email:UseSsl"] = emailEnabled ? "true" : null })); builder.ConfigureTestServices(services => { services.AddSingleton<IUserRepository>(stores.Users); services.AddSingleton<IRefreshTokenRepository>(stores.RefreshTokens); services.AddSingleton<INotificationDeliveryAttemptRepository>(stores.Attempts); services.AddSingleton<IEmergencyContactRepository>(stores.Contacts); services.AddSingleton<IPushNotificationTokenRepository>(stores.Tokens); services.AddSingleton<IFcmPushClient>(stores.FcmClient); services.AddSingleton<IEmailNotificationSender>(stores.EmailSender); }); });
    ...
    private sealed class Stores { public Users Users { get; } = new(); public RefreshTokens RefreshTokens { get; } = new(); public Attempts Attempts { get; } = new(); public Contacts Contacts { get; } = new(); public Tokens Tokens { get; } = new(); public FcmClient FcmClient { get; } = new(); public EmailSender EmailSender { get; } = new(); }
    ...
    private sealed class EmailSender : IEmailNotificationSender { public int Calls { get; private set; } public EmailNotificationMessage? LastMessage { get; private set; } public bool Fail { get; set; } public Task<string?> SendAsync(EmailNotificationMessage message, EmailNotificationProviderOptions options, CancellationToken cancellationToken) { Calls++; LastMessage = message; if (Fail) throw new InvalidOperationException("smtp-secret must not leak"); return Task.FromResult<string?>("email-message"); } }
}
