...
public sealed class AuditLogEndpointsTests
{
    ...
    private sealed class AuditLogs : IAuditLogRepository { public List<AuditLogEntry> Items { get; } = []; public Task AddAsync(AuditLogEntry entry, CancellationToken ct) { Items.Add(entry); return Task.CompletedTask; } public Task<AuditLogEntry?> GetByIdAsync(string id, CancellationToken ct) => Task.FromResult(Items.FirstOrDefault(i => i.Id == id)); public Task<IReadOnlyList<AuditLogEntry>> ListAsync(AuditLogQuery q, CancellationToken ct) => Task.FromResult<IReadOnlyList<AuditLogEntry>>(Apply(q).OrderByDescending(i => i.CreatedAtUtc).Skip((q.PageNumber - 1) * q.PageSize).Take(q.PageSize).ToArray()); public Task<long> CountAsync(AuditLogQuery q, CancellationToken ct) => Task.FromResult((long)Apply(q).Count()); public Task<long> CountOlderThanAsync(DateTimeOffset cutoffUtc, CancellationToken ct) => Task.FromResult((long)Items.Count(i => i.CreatedAtUtc < cutoffUtc)); public Task<long> DeleteOlderThanAsync(DateTimeOffset cutoffUtc, CancellationToken ct) { int count = Items.RemoveAll(i => i.CreatedAtUtc < cutoffUtc); return Task.FromResult((long)count); } private IEnumerable<AuditLogEntry> Apply(AuditLogQuery q) => Items.Where(i => (q.ActorUserId is null || i.ActorUserId == q.ActorUserId) && (!q.Action.HasValue || i.Action == q.Action) && (!q.Module.HasValue || i.Module == q.Module) && (!q.Outcome.HasValue || i.Outcome == q.Outcome) && (q.EntityType is null || i.EntityType == q.EntityType) && (q.EntityId is null || i.EntityId == q.EntityId)); }
}
