...
public sealed class OfflineIngestionEndpointsTests
{
    [Fact]
    public async Task BatchRequiresAuthentication()
    {
        await using WebApplicationFactory<Program> factory = CreateFactory(new TestStores());
        HttpClient client = factory.CreateClient();

        HttpResponseMessage response = await client.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch("m", "t"));
    ...
    public async Task NonRidersReceiveForbidden(string role)
    {
        ...
        HttpClient client = factory.CreateClient();
        await AuthenticateAsync(client, $"offline-{role}@example.com", role, stores);
    ...
    [Fact]
    public async Task RiderCanIngestBatchAndRetryReturnsDuplicate()
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        User user = await AuthenticateAsync(client, "offline-rider@example.com", "Rider", stores);
        (UserDevice mobile, Trip trip) = SeedOperational(stores, user.Id);
        string eventId = Guid.NewGuid().ToString();
        OfflineIngestionBatchRequest request = Batch(mobile.Id, trip.Id, [Item(eventId)]);

        OfflineEnvelope first = (await (await client.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", request)).Content.ReadFromJsonAsync<OfflineEnvelope>())!;
        OfflineEnvelope retry = (await (await client.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", request)).Content.ReadFromJsonAsync<OfflineEnvelope>())!;

        first.Data.Results[0].Status.Should().Be("Accepted");
        retry.Data.Results[0].Status.Should().Be("Duplicate");
        retry.Data.Results[0].AckId.Should().Be(first.Data.Results[0].AckId);
        stores.OfflineRecords.Records.Should().ContainSingle(record => record.ProcessingStatus == OfflineIngestionProcessingStatus.PendingProcessing);
    }

    [Fact]
    public async Task PartialRetryCanMixAcceptedAndDuplicateAndDoesNotReturnPayload()
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        User user = await AuthenticateAsync(client, "offline-mix@example.com", "Rider", stores);
        (UserDevice mobile, Trip trip) = SeedOperational(stores, user.Id);
        string eventId = Guid.NewGuid().ToString();
        await client.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(mobile.Id, trip.Id, [Item(eventId)]));

        HttpResponseMessage response = await client.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(mobile.Id, trip.Id, [Item(eventId), Item(Guid.NewGuid().ToString(), "local-incident")]));
    ...
    [Fact]
    public async Task OwnershipAndValidationAreEnforced()
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient owner = factory.CreateClient();
        HttpClient otherClient = factory.CreateClient();
        User ownerUser = await AuthenticateAsync(owner, "offline-owner@example.com", "Rider", stores);
        User otherUser = await AuthenticateAsync(otherClient, "offline-other@example.com", "Rider", stores);
        (UserDevice ownerMobile, Trip ownerTrip) = SeedOperational(stores, ownerUser.Id);
        (UserDevice otherMobile, Trip otherTrip) = SeedOperational(stores, otherUser.Id);

        (await otherClient.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(ownerMobile.Id, otherTrip.Id))).StatusCode.Should().Be(HttpStatusCode.NotFound);
        (await otherClient.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(otherMobile.Id, ownerTrip.Id))).StatusCode.Should().Be(HttpStatusCode.NotFound);
        (await otherClient.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(otherMobile.Id, otherTrip.Id, []))).StatusCode.Should().Be(HttpStatusCode.BadRequest);
        (await otherClient.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(otherMobile.Id, otherTrip.Id, Enumerable.Range(0, 11).Select(_ => Item(Guid.NewGuid().ToString())).ToArray()))).StatusCode.Should().Be(HttpStatusCode.BadRequest);
        (await otherClient.PostAsJsonAsync("/api/v1/mobile/offline-ingestion/batch", Batch(otherMobile.Id, otherTrip.Id, [Item(Guid.NewGuid().ToString(), "bad-type")]))).StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    private static (UserDevice Mobile, Trip Trip) SeedOperational(TestStores stores, string userId)
    {
        ...
        var trip = new Trip { UserId = userId, VehicleId = "vehicle", MobileDeviceId = mobile.Id, Status = TripStatus.Active, StartedAtUtc = DateTimeOffset.UtcNow, CreatedAtUtc = DateTimeOffset.UtcNow };
        stores.Trips.Trips.Add(trip);
        return (mobile, trip);
    }

    private static OfflineIngestionBatchRequest Batch(string mobileId, string tripId, IReadOnlyList<OfflineIngestionItemRequest>? items = null) => new(Guid.NewGuid().ToString(), mobileId, tripId, 1, DateTimeOffset.UtcNow, "1.0.0", items ?? [Item(Guid.NewGuid().ToString())]);
    private static OfflineIngestionItemRequest Item(string eventId, string type = "minor-event") => new(eventId, type, DateTimeOffset.UtcNow, 1, JsonDocument.Parse("{\"score\":35}").RootElement.Clone());
    ...
    private sealed record OfflineEnvelope(bool Success, OfflineIngestionBatchResponse Data);
}
