...
public sealed class OfflineProcessingEndpointsTests
{
    ...
    public async Task OfflineProcessingEndpointsRequireAuthentication()
    {
        ...
        (await client.GetAsync("/api/v1/admin/offline-processing/worker/status")).StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Theory]
    [InlineData("Monitor")]
    [InlineData("Admin")]
    public async Task NonRidersAreForbiddenFromRiderStatus(string role)
    {
        var stores = new Stores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        await AuthenticateAsync(client, $"offline-processing-{role.ToLowerInvariant()}@example.com", role, stores);

        (await client.GetAsync("/api/v1/offline-processing/status")).StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    [Fact]
    public async Task RiderProcessesOwnRecordsAndDoesNotExposePayloadOrWorkerGlobals()
    {
        var stores = new Stores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient riderClient = factory.CreateClient();
        HttpClient otherClient = factory.CreateClient();
        User rider = await AuthenticateAsync(riderClient, "offline-processing-rider@example.com", "Rider", stores);
        User other = await AuthenticateAsync(otherClient, "offline-processing-other@example.com", "Rider", stores);
        ...
        statusBody.Should().Contain("processed").And.NotContain("workerEnabled").And.NotContain("pendingCount").And.NotContain("maxItemsPerRun");
    ...
    [Fact]
    public async Task AdminWorkerStatusIsAdminOnlyAndReturnsConfiguredSafeGlobalCounts()
    {
        var stores = new Stores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores, new Dictionary<string, string?>
        {
            ...
            ["Mobile:OfflineProcessingWorker:RunOnStartup"] = "false",
        ...
        HttpClient monitor = factory.CreateClient();
        await AuthenticateAsync(admin, "offline-processing-admin@example.com", "Admin", stores);
        User riderUser = await AuthenticateAsync(rider, "offline-processing-rider-status@example.com", "Rider", stores);
        await AuthenticateAsync(monitor, "offline-processing-monitor-status@example.com", "Monitor", stores);
        stores.Records.Items.Add(Record(riderUser.Id, OfflineIngestionItemType.MinorEvent, "{\"secret\":\"do-not-return\"}"));
        OfflineIngestionRecord failed = Record(riderUser.Id, OfflineIngestionItemType.MinorEvent, "{\"secret\":\"failed\"}");
        failed.ProcessingStatus = OfflineIngestionProcessingStatus.FailedPermanent;
        stores.Records.Items.Add(failed);

        string body = await (await admin.GetAsync("/api/v1/admin/offline-processing/worker/status")).Content.ReadAsStringAsync();

        (await rider.GetAsync("/api/v1/admin/offline-processing/worker/status")).StatusCode.Should().Be(HttpStatusCode.Forbidden);
        (await monitor.GetAsync("/api/v1/admin/offline-processing/worker/status")).StatusCode.Should().Be(HttpStatusCode.Forbidden);
        body.Should().Contain("workerEnabled").And.Contain("intervalSeconds").And.Contain("maxItemsPerRun").And.Contain("recoveryMinutes").And.Contain("pendingCount").And.Contain("failedCount").And.NotContain("do-not-return").And.NotContain("payload").And.NotContain("token").And.NotContain("connectionString");
    }
    ...
    public async Task InvalidMaxItemsReturnsValidationError()
    {
        ...
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        await AuthenticateAsync(client, "offline-processing-validation@example.com", "Rider", stores);
    ...
    private static WebApplicationFactory<Program> CreateFactory(Stores stores, IReadOnlyDictionary<string, string?>? workerOptions = null) => new WebApplicationFactory<Program>().WithWebHostBuilder(builder => { builder.UseEnvironment("Testing"); builder.ConfigureAppConfiguration((_, c) => { var values = new Dictionary<string, string?> { ["Jwt:Issuer"] = "MotoSOS", ["Jwt:Audience"] = "MotoSOS.Clients", ["Jwt:Key"] = new string('P', 48), ["Jwt:AccessTokenMinutes"] = "15", ["Jwt:RefreshTokenDays"] = "7", ["Jwt:RefreshTokenRememberMeDays"] = "30", ["MongoDb:ConnectionString"] = string.Empty, ["MongoDb:DatabaseName"] = "MotoSOS_Test" }; if (workerOptions is not null) foreach (KeyValuePair<string, string?> item in workerOptions) values[item.Key] = item.Value; c.AddInMemoryCollection(values); }); builder.ConfigureTestServices(services => { services.AddSingleton<IUserRepository>(stores.Users); services.AddSingleton<IRefreshTokenRepository>(stores.RefreshTokens); services.AddSingleton<IOfflineIngestionRepository>(stores.Records); services.AddSingleton<IIncidentService, Incidents>(); services.AddSingleton<IAlertDispatchService, Alerts>(); services.AddSingleton<ILocationSharingService, Locations>(); }); });
