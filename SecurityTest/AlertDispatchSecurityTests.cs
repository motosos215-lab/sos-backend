...
public sealed class AlertDispatchSecurityTests
{
    [Fact]
    public async Task AlertDispatchResponsesDoNotExposeSensitiveProviderOrNotificationData()
    {
        var stores = new Stores(); await using WebApplicationFactory<Program> factory = CreateFactory(stores); HttpClient client = factory.CreateClient(); User user = await AuthenticateAsync(client, "alert-safe@example.com", "Rider", stores); Incident incident = SeedReadyIncident(stores, user.Id); stores.Devices.Items.Single().DeviceIdentifierHash = "hashed-device-id";

        string body = await (await client.PostAsJsonAsync("/api/v1/alert-dispatches", Request(incident.Id))).Content.ReadAsStringAsync();

        body.Should().Contain("alertDispatch");
        ...
        body.Should().NotContain("WhatsApp");
        body.Should().NotContain("FCM");
    }

    [Fact]
    public async Task CannotCreateReadOrCancelOtherUsersAlertDispatches()
    {
        var stores = new Stores(); await using WebApplicationFactory<Program> factory = CreateFactory(stores); HttpClient owner = factory.CreateClient(); HttpClient attacker = factory.CreateClient(); User ownerUser = await AuthenticateAsync(owner, "alert-owner-sec@example.com", "Rider", stores); User attackerUser = await AuthenticateAsync(attacker, "alert-attacker-sec@example.com", "Rider", stores); Incident ownerIncident = SeedReadyIncident(stores, ownerUser.Id); SeedReadyIncident(stores, attackerUser.Id);
        AlertEnvelope created = (await (await owner.PostAsJsonAsync("/api/v1/alert-dispatches", Request(ownerIncident.Id))).Content.ReadFromJsonAsync<AlertEnvelope>())!;

        (await attacker.PostAsJsonAsync("/api/v1/alert-dispatches", Request(ownerIncident.Id))).StatusCode.Should().Be(HttpStatusCode.NotFound);
        (await attacker.GetAsync($"/api/v1/alert-dispatches/{created.Data.AlertDispatch.Id}")).StatusCode.Should().Be(HttpStatusCode.NotFound);
        (await attacker.PostAsJsonAsync($"/api/v1/alert-dispatches/{created.Data.AlertDispatch.Id}/cancel", new CancelAlertDispatchRequest(null, null))).StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Theory]
    [InlineData("Monitor")]
    [InlineData("Admin")]
    public async Task MonitorAndAdminCannotUseAlertDispatchApi(string role)
    {
        var stores = new Stores(); await using WebApplicationFactory<Program> factory = CreateFactory(stores); HttpClient client = factory.CreateClient(); await AuthenticateAsync(client, $"alert-security-{role}@example.com", role, stores);
        (await client.GetAsync("/api/v1/alert-dispatches")).StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    private static Incident SeedReadyIncident(Stores stores, string userId) { stores.Profiles.Items.Add(new DriverProfile { UserId = userId, CompletionStatus = ProfileCompletionStatus.Completed }); stores.Vehicles.Items.Add(new DriverVehicle { UserId = userId, IsActive = true, CompletionStatus = VehicleCompletionStatus.Completed }); stores.Contacts.Items.Add(new EmergencyContact { UserId = userId, FullName = "Contact", IsActive = true, InvitationStatus = EmergencyContactInvitationStatus.Invited }); stores.Devices.Items.Add(new UserDevice { UserId = userId, DeviceType = DeviceType.MobileApp, IsActive = true, LinkStatus = DeviceLinkStatus.Linked }); stores.Subscriptions.Items.Add(new UserSubscription { UserId = userId, Status = SubscriptionStatus.Active, PlanTier = PlanTier.Basic }); stores.Confirmations.Items.Add(new OnboardingConfirmation { UserId = userId, IsOperational = true }); Incident incident = new() { UserId = userId, TripId = "trip", VehicleId = "vehicle", MobileDeviceId = "mobile", SmartwatchDeviceId = "watch", ClientIncidentId = Guid.NewGuid().ToString(), IdempotencyKey = Guid.NewGuid().ToString(), Source = IncidentSource.MobileDetection, Cause = IncidentCause.CountdownTimeout, RiskLevel = IncidentRiskLevel.High, Status = IncidentStatus.Open, OccurredAtUtc = DateTimeOffset.UtcNow, CreatedAtUtc = DateTimeOffset.UtcNow }; stores.Incidents.Items.Add(incident); return incident; }
    private static CreateAlertDispatchRequest Request(string incidentId) => new(incidentId, Guid.NewGuid().ToString(), "High", "IncidentCreated", DateTimeOffset.UtcNow, "prepared");
