...
public sealed class TripSecurityTests
{
    [Fact]
    public async Task TripResponsesDoNotExposeSensitiveFieldsOrProviderData()
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        User user = await AuthenticateAsync(client, "trip-safe@example.com", "Rider", stores);
        (DriverVehicle vehicle, UserDevice mobile) = SeedReadyState(stores, user.Id);
        mobile.DeviceIdentifierHash = "hashed-device-id";

        await client.PostAsJsonAsync("/api/v1/trips/start", StartRequest(vehicle.Id, mobile.Id));
        string body = await (await client.GetAsync("/api/v1/trips/active")).Content.ReadAsStringAsync();
    ...
    [Fact]
    public async Task CannotReadFinishOrStartTripsWithOtherUsersResources()
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient owner = factory.CreateClient();
        HttpClient attacker = factory.CreateClient();
        User ownerUser = await AuthenticateAsync(owner, "trip-owner@example.com", "Rider", stores);
        User attackerUser = await AuthenticateAsync(attacker, "trip-attacker@example.com", "Rider", stores);
        (DriverVehicle ownerVehicle, UserDevice ownerMobile) = SeedReadyState(stores, ownerUser.Id);
        SeedReadyState(stores, attackerUser.Id);
        var trip = new Trip { UserId = ownerUser.Id, VehicleId = ownerVehicle.Id, MobileDeviceId = ownerMobile.Id, Status = TripStatus.Active, StartedAtUtc = DateTimeOffset.UtcNow, CreatedAtUtc = DateTimeOffset.UtcNow };
        stores.Trips.Trips.Add(trip);

        HttpResponseMessage read = await attacker.GetAsync($"/api/v1/trips/{trip.Id}");
        HttpResponseMessage finish = await attacker.PostAsJsonAsync($"/api/v1/trips/{trip.Id}/finish", new FinishTripRequest(null, null, null, null));
        HttpResponseMessage startWithOtherVehicle = await attacker.PostAsJsonAsync("/api/v1/trips/start", StartRequest(ownerVehicle.Id, ownerMobile.Id));

        read.StatusCode.Should().Be(HttpStatusCode.NotFound);
        finish.StatusCode.Should().Be(HttpStatusCode.NotFound);
        startWithOtherVehicle.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    [Theory]
    [InlineData("Monitor")]
    [InlineData("Admin")]
    public async Task MonitorAndAdminCannotUseTripsApi(string role)
    {
        var stores = new TestStores();
        await using WebApplicationFactory<Program> factory = CreateFactory(stores);
        HttpClient client = factory.CreateClient();
        await AuthenticateAsync(client, $"trip-{role}@example.com", role, stores);
    ...
    private static (DriverVehicle Vehicle, UserDevice Mobile) SeedReadyState(TestStores stores, string userId)
    {
        ...
        var vehicle = new DriverVehicle { UserId = userId, IsActive = true, CompletionStatus = VehicleCompletionStatus.Completed };
    ...
    private static StartTripRequest StartRequest(string vehicleId, string mobileId) => new(vehicleId, mobileId, null, DateTimeOffset.UtcNow, null, 87, "1.0.0");
    ...
    private static WebApplicationFactory<Program> CreateFactory(TestStores stores) => new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
    {
        ...
        builder.ConfigureAppConfiguration((_, configuration) => configuration.AddInMemoryCollection(new Dictionary<string, string?> { ["Jwt:Issuer"] = "MotoSOS", ["Jwt:Audience"] = "MotoSOS.Clients", ["Jwt:Key"] = new string('R', 48), ["Jwt:AccessTokenMinutes"] = "15", ["Jwt:RefreshTokenDays"] = "7", ["Jwt:RefreshTokenRememberMeDays"] = "30", ["MongoDb:ConnectionString"] = string.Empty, ["MongoDb:DatabaseName"] = "MotoSOS_Test" }));
