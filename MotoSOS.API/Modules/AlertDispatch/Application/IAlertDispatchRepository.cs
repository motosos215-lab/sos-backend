...
public interface IAlertDispatchRepository
{
    ...
    Task<IReadOnlyList<AlertDispatchRequest>> ListCandidatesForAutomaticEscalationAsync(DateTimeOffset cutoffUtc, int maxItems, CancellationToken cancellationToken) => Task.FromResult<IReadOnlyList<AlertDispatchRequest>>([]);
    Task<IReadOnlyList<AlertDispatchRequest>> ListByIncidentIdAsync(string userId, string incidentId, CancellationToken cancellationToken) => ListByUserIdAsync(userId, null, incidentId, 1, int.MaxValue, cancellationToken);
