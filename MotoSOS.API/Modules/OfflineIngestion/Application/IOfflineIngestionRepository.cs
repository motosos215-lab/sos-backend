...
public interface IOfflineIngestionRepository
{
    ...
    Task<IReadOnlyList<OfflineIngestionRecord>> ListPendingAsync(int maxItems, CancellationToken cancellationToken) => Task.FromResult<IReadOnlyList<OfflineIngestionRecord>>([]);
    ...
    Task<int> RecoverStaleProcessingAsync(DateTimeOffset cutoffUtc, DateTimeOffset now, int maxItems, CancellationToken cancellationToken) => Task.FromResult(0);
    ...
    Task<long> CountByStatusAsync(OfflineIngestionProcessingStatus status, CancellationToken cancellationToken) => Task.FromResult(0L);
