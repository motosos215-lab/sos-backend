...
public static class EmergencyEscalationEndpoints
{
    public static IEndpointRouteBuilder MapEmergencyEscalationEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/api/v1/admin/escalations/worker/status", async (ClaimsPrincipal principal, IAutomaticEscalationService service, CancellationToken ct) => { string? userId = GetUserId(principal); if (string.IsNullOrWhiteSpace(userId)) return Results.Unauthorized(); return Results.Ok(ApiResponse<AutomaticEscalationWorkerStatusResponse>.Ok(await service.GetWorkerStatusAsync(userId, ct))); }).RequireAuthorization().WithTags("EmergencyEscalations");
        endpoints.MapPost("/api/v1/admin/escalations/worker/run", async (RunAutomaticEscalationRequest request, ClaimsPrincipal principal, IAutomaticEscalationService service, CancellationToken ct) => { string? userId = GetUserId(principal); if (string.IsNullOrWhiteSpace(userId)) return Results.Unauthorized(); if (request.MaxItems is < 1 or > 100) return Results.BadRequest(ApiResponse<object>.Fail(new ApiError("validation_error", "maxItems must be between 1 and 100."))); if (request.EscalateAfterSeconds is < 60) return Results.BadRequest(ApiResponse<object>.Fail(new ApiError("validation_error", "escalateAfterSeconds must be greater than or equal to 60."))); return Results.Ok(ApiResponse<RunAutomaticEscalationResponse>.Ok(await service.RunForAdminAsync(userId, request, ct))); }).RequireAuthorization().WithTags("EmergencyEscalations");
        return endpoints;
    }
