...
public sealed class OfflineProcessingWorkerServiceTests
{
    ...
    private sealed class SosAlerts : ICreateSosAlertService { public Task<CreateSosAlertResponse> CreateAsync(string userId, CreateSosAlertRequest request, CancellationToken ct) => Task.FromResult(new CreateSosAlertResponse(new SosAlertIncidentResponse("incident-sos-1", request.TripId!, "Open", request.IncidentType ?? "ManualSos", request.Severity ?? "High"), new SosAlertDispatchResponse("alert-sos-1", "incident-sos-1", "PendingDispatch", 1), [new SosAlertNotificationAttemptResponse("attempt-1", "Push", "Prepared", "None", "contact-1", "Monitor")], new SosAlertSummaryResponse(1, 0, 0, 1))); }
}
