...
public sealed class NotificationServiceTests
{
    ...
    public async Task PrepareCreatesPushAttemptForLinkedContactWithActiveFcmToken()
    {
        ...
        response.Attempts.Select(a => a.Channel).Should().Equal("Push", "Sms");
    ...
    private static NotificationService Service(User user, bool ready = true, Alerts? alerts = null, Attempts? attempts = null, Contacts? contacts = null, PushTokens? pushTokens = null) => new(new Users(user), new StubOnboarding(ready), alerts ?? new Alerts(), attempts ?? new Attempts(), new NotificationIdempotencyKeyFactory(), contacts ?? new Contacts(), pushTokens ?? new PushTokens(), new Clock());
