...
public sealed class AuditLogServiceTests
{
    ...
    private sealed class FailingAuditLogs : IAuditLogRepository { public Task AddAsync(AuditLogEntry entry, CancellationToken ct) => throw new InvalidOperationException("store failed"); public Task<AuditLogEntry?> GetByIdAsync(string id, CancellationToken ct) => Task.FromResult<AuditLogEntry?>(null); public Task<IReadOnlyList<AuditLogEntry>> ListAsync(AuditLogQuery query, CancellationToken ct) => Task.FromResult<IReadOnlyList<AuditLogEntry>>([]); public Task<long> CountAsync(AuditLogQuery query, CancellationToken ct) => Task.FromResult(0L); }
}
